...
{
$ti_error = TrustindexPlugin::___('Wrong e-mail or password! <a href="%s">You can try again!</a>', array(admin_url('admin.php?page='.$trustindex_pm_trustpilot->get_plugin_slug().'/settings.php&tab=setup_trustindex')));
}
}
else
{
...
<?php if ($ti_success == "connected"): ?>
	...
	<?php echo $trustindex_pm_trustpilot->get_noticebox("success", TrustindexPlugin::___('Trustindex account successfully connected!')); ?>

<?php elseif ($ti_success == "disconnected"): ?>

	<?php echo $trustindex_pm_trustpilot->get_noticebox("success", TrustindexPlugin::___('Trustindex account successfully disconnected!')); ?>

<?php endif; ?>

<?php if ($ti_error): ?>

	<?php echo $trustindex_pm_trustpilot->get_noticebox("error", $ti_error); ?>

<?php endif; ?>

<div class="maxwidth">
<h1><?php  echo TrustindexPlugin::___('You can connect your %s with your Trustindex account, and can display your widgets easier.', [ 'Widgets for Trustpilot Reviews' ]); ?></h1>
<ul class="numbered">
<li>
...
- <?php  echo TrustindexPlugin::___('Your package:'); ?> <strong><?php  echo TrustindexPlugin::___($ti_package); ?></strong>
			...
			<?php if ($ti_package == "free"): ?>
				...
				<?php echo $trustindex_pm_trustpilot->get_noticebox("error", TrustindexPlugin::___('Your widgets will not display fully after the trial period is over!')); ?>

			<?php elseif ($ti_package == "trial"): ?>

				<?php echo $trustindex_pm_trustpilot->get_noticebox("warning", TrustindexPlugin::___('Your widgets will not display, because the trial period is over! You can subscribe or switch back the the "%s" tab', [ TrustindexPlugin::___('Free Widget Configurator') ])); ?>
...
<br />
<p class="text-center">
<a class="btn-text" href="https://admin.trustindex.io/forgot-password" target="_blank"><?php  echo TrustindexPlugin::___('Have you forgotten your password?'); ?></a>
...
<a class="btn-text" href="https://admin.trustindex.io/widget" target="_blank" <?php  if ($ti_success == "connected"): ?>data-autoclick="true"<?php  endif; ?>><?php  echo TrustindexPlugin::___("Go to Trustindex's admin!"); ?></a>
		<?php if ($ti_success == "connected"): ?>
			...
			<?php echo $trustindex_pm_trustpilot->get_noticebox("success", TrustindexPlugin::___('We will redirect you to the admin panel automatically in some seconds...')); ?>
...
<p>
					<?php echo $trustindex_pm_trustpilot->get_noticebox("warning", "You have no widgets saved!"); ?>
