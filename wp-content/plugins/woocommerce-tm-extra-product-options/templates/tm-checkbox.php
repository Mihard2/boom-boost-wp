...
?>
<li class="tmcp-field-wrap<?php echo esc_attr( $grid_break . $li_class ); ?><?php if( ! empty( $label_mode ) ) {echo ' tc-mode-' . esc_attr( $label_mode ); } ?> option-checkbox">
    ...
    <label class="tm-epo-field-label" for="<?php echo esc_attr( $id ); ?>">
        ...
        <span class="tm-epo-style-wrapper <?php echo esc_attr( $labelclass_start ); ?> option-checkbox__title">
		<?php } ?>
	...
	<?php
			...
			echo '<span class="tc-label-wrap' . ( empty( $hexclass ) ? '' : ' ' . $hexclass ) . ' option-checkbox__title">';
			if ( empty( $use_images ) || ( isset( $use_images ) && $use_images != "images" ) ) {
				if ( empty( $use_images ) ) {
